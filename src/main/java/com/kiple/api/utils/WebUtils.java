...
public class WebUtils {
    ...
    private static final String DEFAULT_CHARSET = KiplePayConstants.CHARSET_UTF8;
     ...
     * @throws IOException
     */
    public static String doPost(String url, RequestParametersHolder request, String charset,
                                int connectTimeout, int readTimeout, String proxyHost,
                                int proxyPort, String postType) throws IOException {
        //String ctype = "application/x-www-form-urlencoded;charset=" + charset;
        String ctype = "";
        if(PostTypeEnums.POST_APPLICATION_JSON.getType().equals(postType)){
            ctype = "application/json;charset=" + charset;
        }else if(PostTypeEnums.POST_MULTIPART_FORM_DATA.getType().equals(postType)){
            ctype = "multipart/form-data";
        }else if(PostTypeEnums.POST_FORM_DATA.getType().equals(postType)){
            ctype = "application/x-www-form-urlencoded;charset=" + charset;
        }

        return doRequest(url,request,charset,connectTimeout,readTimeout,proxyHost,proxyPort,ctype);
    }
     ...
     * @throws IOException
     */
    private static String doRequest(String url, RequestParametersHolder request, String charset,
                                    int connectTimeout, int readTimeout, String proxyHost,
                                    int proxyPort,String ctype) throws IOException{

        String query = "";
        byte[] contentBytes = {};
        //upload file
        if(PostTypeEnums.POST_MULTIPART_FORM_DATA.getType().equals(ctype)){
            KiplePayHashMap map = new KiplePayHashMap();
            //业务参数 一个key里面
            map.put(KiplePayConstants.BIZ_CONTENT_KEY,buildQuery(request.getApplicationParams(), charset));
            if(request.getApplicationParams() != null && !request.getApplicationParams().isEmpty()){
                String content = request.getApplicationParams().get(KiplePayConstants.BIZ_CONTENT_KEY);
                JSONObject jsonObject = JsonUtils.getJSONObject(content);
                if(jsonObject != null){
                    //上传文件的时候 请求之前需要解析参数 gateway hard to handle it
                    for (Entry<String, Object> stringObjectEntry : jsonObject.entrySet()) {
                        map.put(stringObjectEntry.getKey(),stringObjectEntry.getValue());
                    }
                }
            }
            return doUploadFilePost(url, ctype, map, connectTimeout, readTimeout,
                    proxyHost, proxyPort,request.getFilesPath(),request.getHeaderMap());

            //使用json的方式访问到网关 容易导致参数乱序 导致签名失败
//        }else if(ctype.startsWith(PostTypeEnums.POST_APPLICATION_JSON.getType())){
//                query = request.getApplicationParams().get(KiplePayConstants.BIZ_CONTENT_KEY);
//                if(!StringUtils.isEmpty(query)){
//                    Map map = new HashMap();
//                    map.put(KiplePayConstants.BIZ_CONTENT_KEY,JSONObject.parseObject(query));
//                    contentBytes = JSON.toJSONString(map).getBytes(charset);
//                }
        }else{
            query = buildQuery(request.getApplicationParams(), charset);
            if (!StringUtils.isEmpty(query)) {
                contentBytes = query.getBytes(charset);
            }
        }
        return doPost(url, ctype, contentBytes, connectTimeout, readTimeout, proxyHost, proxyPort,request.getHeaderMap());
    }
...
//        String ctype = "application/x-www-form-urlencoded;charset=" + charset;
//        return doRequest(url,params,charset,connectTimeout,readTimeout,proxyHost,proxyPort,ctype);
//    }
    ...
    public static String doPost(String url, String ctype, byte[] content, int connectTimeout,
                                int readTimeout, String proxyHost, int proxyPort,KiplePayHashMap headerMap) throws IOException {
        ...
        try {
            try {
                ...
                //add customer header parameters
                if(headerMap != null){
                ...
                KiplePayLogger.logCommError(e, url, map.get("app_key"), map.get("method"), content);
                throw e;
            }
            try {
                ...
                Map<String, String> map = getParamsFromUrl(url);
                KiplePayLogger.logCommError(e, conn, map.get("app_key"), map.get("method"), content);
                throw e;
            }
    ...
    public static String doUploadFilePost(String url, String ctype, Map<String, String> paramter, int connectTimeout,
                                int readTimeout, String proxyHost, int proxyPort,String[] filePath,KiplePayHashMap headerMap) throws IOException {
        HttpURLConnection conn = null;
        OutputStream out = null;
        String rsp = null;
        ...
        try {
            try {
                ...
                Map<String, String> map = getParamsFromUrl(url);
                KiplePayLogger.logCommError(e, url, map.get("app_key"), map.get("method"), "upload".getBytes());
                throw e;
            }
            try {
                out = conn.getOutputStream();
                ...
                // 请求参数部分
                writeParams(paramter, os,boundary);
                // 请求上传文件部分
                if (filePath != null){
                    writeFile(filePath, os,boundary);
                }
                // 请求结束标志
                ...
                Map<String, String> map = getParamsFromUrl(url);
                KiplePayLogger.logCommError(e, conn, map.get("app_key"), map.get("method"), "upload".getBytes());
                throw e;
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    private static void writeParams(Map<String, String> requestText,
                                    OutputStream os,String boundary) throws Exception {
     ...
     *
     * */
    private static void writeFile(String[] filePaths,
                                  OutputStream os,String boundary) throws Exception {
            for (String filePath : filePaths) {

                StringBuilder requestParams = new StringBuilder();
                String[] fileInfo =  filePath.split(",",2);
                //check file
                KiplePayFileUpload kiplePayFileUpload = IOUtils.readFile(fileInfo[1]);

                requestParams.append(PREFIX).append(boundary).append(LINE_END);
                requestParams.append("Content-Disposition: form-data; name=\"")
                        .append(fileInfo[0]).append("\"; filename=\"")
                        .append(kiplePayFileUpload.getFileName()).append("\"")
                        .append(LINE_END);
                requestParams.append("Content-Type:")
                        .append(kiplePayFileUpload.getContentType())
                        .append(LINE_END);
                requestParams.append("Content-Transfer-Encoding: 8bit").append(
                        LINE_END);
                ...
                os.write(requestParams.toString().getBytes());
                os.write(kiplePayFileUpload.getFileBytes());
     ...
     * @throws IOException
     */
    public static String doGet(String url, Map<String, String> params,KiplePayHashMap headerMap) throws IOException {
    ...
    public static String doGet(String url, Map<String, String> params,
                               String charset,KiplePayHashMap headerMap) throws IOException {
        HttpURLConnection conn = null;
        String rsp = null;
        try {
            String ctype = "application/x-www-form-urlencoded;charset=" + charset;
            ...
            try {
                ...
                Map<String, String> map = getParamsFromUrl(url);
                KiplePayLogger.logCommError(e, url, map.get("app_key"), map.get("method"), params);
                throw e;
            }
            //add customer header parameters
            if(headerMap != null){
            ...
            try {
                ...
                Map<String, String> map = getParamsFromUrl(url);
                KiplePayLogger.logCommError(e, conn, map.get("app_key"), map.get("method"), params);
                throw e;
            }
