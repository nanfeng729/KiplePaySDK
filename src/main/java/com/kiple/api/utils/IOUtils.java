...
import com.kiple.api.domain.request.KiplePayFileUpload;
import com.kiple.api.exception.KiplePayApiException;
import com.kiple.api.exception.KiplePayApiRunTimeException;
 ...
 * </ul>
 * For example, {@link #copy(InputStream, OutputStream)} calls {@link #copyLarge(InputStream, OutputStream)} which calls {@link
 * #copy(InputStream, OutputStream, int)} which creates the buffer and calls {@link #copyLarge(InputStream, OutputStream, byte[])}.
 * <p>
 * Applications can re-use buffers by using the underlying methods directly. This may improve performance for applications that need to do a
 * lot of copying.
 * <p>
...
public class IOUtils {
     ...
     * <p>
     * This is typically used in finally blocks to ensure that the closeable is closed even if an Exception was thrown before the normal
     * close statement was reached.
     * <br>
     * <b>It should not be used to replace the close statement(s)
     * which should be present for the non-exceptional case.</b>
     * <br>
     * It is only intended to simplify tidying up where normal processing has already failed and reporting close failure as well is not
     * necessary or useful.
     * <p>
     * Example code:
     * </p>
     ...
     * </ul>
     * It can be used in favor of {@link #toByteArray(InputStream)}, since it avoids unnecessary allocation and copy of byte[].<br> This
    ...
    /**
     * Returns the given OutputStream if it is already a {@link BufferedOutputStream}, otherwise creates a BufferedOutputStream from the
     * given OutputStream.
     *
    ...
    /**
     * Gets contents of an <code>InputStream</code> as a <code>byte[]</code>. Use this method instead of
     * <code>toByteArray(InputStream)</code> when <code>InputStream</code> size is known.
     * <b>NOTE:</b> the method checks that the length can safely be cast to an int without truncation
     * before using {@link IOUtils#toByteArray(InputStream, int)} to read into the byte array. (Arrays can have no more than
    ...
    /**
     * Gets the contents of a <code>Reader</code> as a <code>byte[]</code> using the default character encoding of the platform.
     * <p>
     * This method buffers the input internally, so there is no need to use a
    ...
    /**
     * Gets the contents of a <code>Reader</code> as a <code>byte[]</code> using the specified character encoding.
     * <p>
     * This method buffers the input internally, so there is no need to use a
     * <code>BufferedReader</code>.
     *
     ...
     * @throws NullPointerException                         if the input is null
     * @throws IOException                                  if an I/O error occurs
     * @throws java.nio.charset.UnsupportedCharsetException thrown instead of {@link java.io .UnsupportedEncodingException} in version 2.2
    ...
    /**
     * Gets the contents of a <code>String</code> as a <code>byte[]</code> using the default character encoding of the platform.
     * <p>
    ...
    /**
     * Gets the contents of an <code>InputStream</code> as a character array using the default character encoding of the platform.
     * <p>
     * This method buffers the input internally, so there is no need to use a
     * <code>BufferedInputStream</code>.
     *
    ...
    /**
     * Gets the contents of an <code>InputStream</code> as a character array using the specified character encoding.
     * <p>
     * This method buffers the input internally, so there is no need to use a
     * <code>BufferedInputStream</code>.
     *
    ...
    /**
     * Gets the contents of an <code>InputStream</code> as a String using the default character encoding of the platform.
     * <p>
     * This method buffers the input internally, so there is no need to use a
     * <code>BufferedInputStream</code>.
     *
    ...
    /**
     * Gets the contents of an <code>InputStream</code> as a list of Strings, one entry per line, using the default character encoding of
     * the platform.
     * <p>
     * This method buffers the input internally, so there is no need to use a
     * <code>BufferedInputStream</code>.
    ...
    /**
     * Writes bytes from a <code>byte[]</code> to an <code>OutputStream</code> using chunked writes. This is intended for writing very large
     * byte arrays which might otherwise cause excessive memory usage if the native code has to allocate a copy.
     *
     * @param data   the byte array to write, do not modify during output, null ignored
     * @param output the <code>OutputStream</code> to write to
     * @throws NullPointerException if output is null
    ...
    /**
     * Writes chars from a <code>char[]</code> to a <code>Writer</code> using chunked writes. This is intended for writing very large byte
     * arrays which might otherwise cause excessive memory usage if the native code has to allocate a copy.
     *
     * @param data   the char array to write, do not modify during output, null ignored
     * @param output the <code>Writer</code> to write to
     * @throws NullPointerException if output is null
    ...
    /**
     * Writes the <code>toString()</code> value of each item in a collection to an <code>OutputStream</code> line by line, using the default
    ...
    /**
     * Writes the <code>toString()</code> value of each item in a collection to an <code>OutputStream</code> line by line, using the
     * specified character encoding and the specified line ending.
     *
     * @param lines      the lines to write, null entries produce blank lines
     * @param lineEnding the line separator to use, null is system default
     * @param output     the <code>OutputStream</code> to write to, not null, not closed
    ...
    /**
     * Writes the <code>toString()</code> value of each item in a collection to a <code>Writer</code> line by line, using the specified line
     * ending.
     *
     * @param lines      the lines to write, null entries produce blank lines
     * @param lineEnding the line separator to use, null is system default
     ...
     * number of bytes cannot be returned as an int. For large streams use the <code>copyLarge(InputStream, OutputStream)</code> method.
     *
     * @param input  the <code>InputStream</code> to read from
     * @param output the <code>OutputStream</code> to write to
    ...
    /**
     * Copies bytes from an <code>InputStream</code> to an <code>OutputStream</code> using an internal buffer of the given size.
     * <p>
     ...
     * </p>
     * <p>
     * Note that the implementation uses {@link #skip(InputStream, long)}. This means that the method may be considerably less efficient
     * than using the actual skip implementation, this is done to guarantee that the correct number of characters are skipped.
     * </p>
     * The buffer size is given by {@link #DEFAULT_BUFFER_SIZE}.
     *
     * @param input       the <code>InputStream</code> to read from
    ...
    /**
     * Skips bytes from an input byte stream. This implementation guarantees that it will read as many bytes as possible before giving up;
     * this may not always be the case for skip() implementations in subclasses of {@link InputStream}.
     * <p>
     * Note that the implementation uses {@link InputStream#read(byte[], int, int)} rather than delegating to {@link
     * InputStream#skip(long)}. This means that the method may be considerably less efficient than using the actual skip implementation,
    ...
    /**
     * Skips characters from an input character stream. This implementation guarantees that it will read as many characters as possible
     * before giving up; this may not always be the case for skip() implementations in subclasses of {@link Reader}.
     * <p>
     * Note that the implementation uses {@link Reader#read(char[], int, int)} rather than delegating to {@link Reader#skip(long)}. This
     * means that the method may be considerably less efficient than using the actual skip implementation, this is done to guarantee that
     * the correct number of characters are skipped.
     * </p>
     *
     ...
     * <p>
     * This allows for the possibility that {@link InputStream#skip(long)} may not skip as many bytes as requested (most likely because of
     * reaching EOF).
     * <p>
     * Note that the implementation uses {@link #skip(InputStream, long)}. This means that the method may be considerably less efficient
     * than using the actual skip implementation, this is done to guarantee that the correct number of characters are skipped.
     ...
     * <p>
     * This allows for the possibility that {@link Reader#skip(long)} may not skip as many characters as requested (most likely because of
     * reaching EOF).
     * <p>
     * Note that the implementation uses {@link #skip(Reader, long)}. This means that the method may be considerably less efficient than
     * using the actual skip implementation, this is done to guarantee that the correct number of characters are skipped.
     * </p>
     *
     * @param input  stream to skip
    ...
    /**
     * Reads characters from an input character stream. This implementation guarantees that it will read as many characters as possible
     * before giving up; this may not always be the case for subclasses of {@link Reader}.
     *
    ...
    /**
     * Reads bytes from an input stream. This implementation guarantees that it will read as many bytes as possible before giving up; this
     * may not always be the case for subclasses of {@link InputStream}.
     *
     * @param input  where to read input from
     * @param buffer destination
     * @param offset initial offset into buffer
     ...
     * <p>
     * This implementation guarantees that it will read as many bytes as possible before giving up; this may not always be the case for
     ...
     * <p>
     * This allows for the possibility that {@link Reader#read(char[], int, int)} may not read as many characters as requested (most likely
     * because of reaching EOF).
     *
     * @param input  where to read input from
     * @param buffer destination
     * @param offset initial offset into buffer
     ...
     * <p>
     * This allows for the possibility that {@link InputStream#read(byte[], int, int)} may not read as many bytes as requested (most likely
     * because of reaching EOF).
     *
     * @param input  where to read input from
     * @param buffer destination
     ...
     * Reads the requested number of bytes or fail if there are not enough left.
     * <p>
     * This allows for the possibility that {@link ReadableByteChannel#read(ByteBuffer)} may not read as many bytes as requested (most
    ...
    /**
     * Gets the file string according to file path
     * @param path
     * @return
     * @throws KiplePayApiException
     */
    public static KiplePayFileUpload readFile(String path) throws KiplePayApiException {
        //default max size 3m
        return readFile(path,3*1024*1024);
    }

    private static KiplePayFileUpload readFile(String path, int maxSize) throws KiplePayApiException {
        KiplePayFileUpload kiplePayFileUpload;

        try(FileInputStream input = new FileInputStream(path);
            java.io.ByteArrayOutputStream output = new java.io.ByteArrayOutputStream()){

            kiplePayFileUpload = new KiplePayFileUpload();
            kiplePayFileUpload.setFileName(getFileName(path));
            kiplePayFileUpload.setContentType(path);

            //check size
            if(input.available() > maxSize){
                throw new KiplePayApiException("send upload file failed,file size greater than "+maxSize);
            }
            ...
            kiplePayFileUpload.setFileBytes(output.toByteArray());
        }catch (Exception e){
            throw new KiplePayApiException(e);
        }
        return kiplePayFileUpload;
    }
    ...
    public static String getContentType(String fileUrl) {
        try {
            return new MimetypesFileTypeMap().getContentType(new File(fileUrl));
        } catch (Exception e) {
            throw new KiplePayApiRunTimeException("get file content type error,path"+fileUrl);
        }
    }
