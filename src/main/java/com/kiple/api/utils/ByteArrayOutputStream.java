...
import static com.kiple.api.utils.IOUtils.EOF;
 ...
 * <code>toString()</code>.
 * <p>
 * Closing a {@code ByteArrayOutputStream} has no effect. The methods in this class can be called after the stream has been closed without
 * generating an {@code IOException}.
 * <p>
 * This is an alternative implementation of the {@link java.io.ByteArrayOutputStream} class. The original implementation only allocates 32
 * bytes at the beginning. As this class is designed for heavy duty it starts at 1024 bytes. In contrast to the original it doesn't
 * reallocate the whole memory block but allocates additional buffers. This way no buffers need to be garbage collected and the contents
 * don't have to be copied to the new buffer. This class is designed to behave exactly like the original. The only exception is the
...
public class ByteArrayOutputStream extends OutputStream {
    ...
    /**
     * Closing a {@code ByteArrayOutputStream} has no effect. The methods in this class can be called after the stream has been closed
     * without generating an {@code IOException}.
     *
     * @throws IOException never (this method should not declare this exception but it has to now due to backwards compatibility)
     */
    @Override
     ...
     * </ul>
     * It can be used in favor of {@link #toByteArray()}, since it avoids unnecessary allocation and copy of byte[].<br> This method buffers
    ...
    /**
     * Gets the current contents of this byte stream as a Input Stream. The returned stream is backed by buffers of <code>this</code>
     * stream, avoiding memory allocation and copy, thus saving space and time.<br>
     *
