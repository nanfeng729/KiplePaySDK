...
public class KiplePayLogger {
    ...
    public static void setNeedEnableLogger(boolean needEnableLogger) {
        KiplePayLogger.needEnableLogger = needEnableLogger;
    }
    ...
    private static void _logCommError(Exception e, HttpURLConnection conn, String url,
        ...
        String sdkName = KiplePayConstants.SDK_VERSION;
        String urlStr = null;
        String rspCode = "";
        if (conn != null) {
            try {
     ...
     * 发生特别错误时记录完整错误现场
     */
    public static void logErrorScene(Map<String, Object> rt, KiplePayResponse tRsp,
                                     String appSecret) {
        if (!needEnableLogger) {
            return;
        }
        ...
        appendLog((KiplePayHashMap) rt.get("protocalMustParams"), sb);
        sb.append("^_^");
        sb.append("ProtocalOptParams:");
        appendLog((KiplePayHashMap) rt.get("protocalOptParams"), sb);
        sb.append("^_^");
        sb.append("ApplicationParams:");
        appendLog((KiplePayHashMap) rt.get("textParams"), sb);
        sb.append("^_^");
        sb.append("Body:");
     ...
     * 发生特别错误时记录完整错误现场
     */
    public static void logBizSummary(Map<String, Object> rt, KiplePayResponse tRsp,
    ...
    private static void appendLog(KiplePayHashMap map, StringBuilder sb) {
        boolean first = true;
