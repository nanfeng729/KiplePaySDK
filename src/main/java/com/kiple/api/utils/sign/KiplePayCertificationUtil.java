 ...
 */
public class KiplePayCertificationUtil {
    ...
    public static boolean isTrusted(String cert, String rootCertContent) {
        ...
        try {
            ...
            KiplePayLogger.logBizError(("读取证书失败"));
        ...
        try {
            ...
            KiplePayLogger.logBizError(("读取根证书失败"));
            throw new RuntimeException(e);
        }
    ...
    private static boolean verifyCert(X509Certificate cert, X509Certificate[] rootCerts) {
        try {
            ...
            KiplePayLogger.logBizError(("证书已经过期"));
            return false;
        } catch (CertificateNotYetValidException e) {
            KiplePayLogger.logBizError(("证书未激活"));
            return false;
        }
        ...
        if (issuer == null) {
            KiplePayLogger.logBizError(("证书链验证失败"));
            return false;
        }
        try {
    ...
    private static boolean verifyCertChain(X509Certificate[] certs, X509Certificate[] rootCerts) {
        ...
        if (!sorted) {
            KiplePayLogger.logBizError(("证书链验证失败：不是完整的证书链"));
            return false;
        }
    ...
    public static String getRootCertSN(String rootCertContent) {
        ...
        try {
            ...
            KiplePayLogger.logBizError(("提取根证书失败"));
        }
        return rootCertSN;
    ...
    public static String getRootCertSN(String rootCertContent, String signType) {
        if (KiplePayConstants.SIGN_TYPE_SM2.equals(signType)) {
            String rootCertSN = null;
            try {
                X509Certificate[] x509Certificates = readPemCertChain(rootCertContent);
                MessageDigest md = MessageDigest.getInstance("MD5");
     ...
     */
    public static X509Certificate getCertFromPath(String certPath) throws KiplePayApiException {
    ...
    public static X509Certificate getCertFromContent(String certContent) throws KiplePayApiException {
        try {
    ...
    public static String getCertSN(X509Certificate cf) throws KiplePayApiException {
        try {
            MessageDigest md = MessageDigest.getInstance("MD5");
    ...
    public static String getPrivateKeyContent(String certContent) throws KiplePayApiException {
        try {
            InputStream inputStream = new ByteArrayInputStream(certContent.getBytes());
            CertificateFactory cf = CertificateFactory.getInstance("X.509", "BC");
            X509Certificate cert= (X509Certificate) cf.generateCertificate(inputStream);
            PublicKey publicKey = cert.getPublicKey();
            return Base64.encodeBase64(publicKey.getEncoded(),true).toString();
        } catch (Exception e) {
            throw new KiplePayApiException(e);
        }
    }

    public static String getKiplePayPublicKey(String alipayPublicCertPath) throws KiplePayApiException {
        InputStream inputStream = null;
        try {
