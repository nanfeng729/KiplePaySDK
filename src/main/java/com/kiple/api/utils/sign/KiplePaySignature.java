...
/**
 * @classname: KiplePaySignature
 * @description: 描述
 * @author: seven.zhang
 * @date: 2020/6/3 18:53
 */
public class KiplePaySignature {
    /**
     * 通用签名方法
     *
     * @param content    待签名内容
    ...
    public static String sign(String content, String privateKey, String charset,
                              String signType) throws KiplePayApiException {
    ...
    public static String signWithPlainText(String str, String privateKey, String charset,
                              String signType) throws KiplePayApiException {

        return AsymmetricManager.getByName(signType).sign(str, charset, privateKey);
    }
    ...
    public static boolean verifyV1(Map<String, String> params, String publicKey,
                                   String charset, String signType) throws KiplePayApiException {
     ...
     *
     * @param params               待验签的从支付宝接收到的参数Map
     * @param KiplePayPublicCertPath 支付宝公钥路径
     * @param charset              参数内容编码集
     * @param signType             指定采用的签名方式，RSA、RSA2、SM2
     * @return true：验签通过；false：验签不通过
     * @throws KiplePayApiException
     */
    public static boolean certVerifyV1(Map<String, String> params, String KiplePayPublicCertPath,
                                       String charset, String signType) throws KiplePayApiException {
        String publicKey = getKiplePayPublicKey(KiplePayPublicCertPath);
     ...
     * @throws KiplePayApiException
     */
    public static boolean certVerifyV2(Map<String, String> params, String KiplePayPublicCertPath,
                                       String charset, String signType) throws KiplePayApiException {
        String publicKey = getKiplePayPublicKey(KiplePayPublicCertPath);
     ...
     * @throws KiplePayApiException
     */
    public static boolean certVerify(String content, String sign, String KiplePayPublicCertPath, String charset,
                                     String signType) throws KiplePayApiException {
        String publicKey = getKiplePayPublicKey(KiplePayPublicCertPath);
     ...
     * @param KiplePayPublicKey 支付宝公钥
     * @param cusPrivateKey   商户私钥
     * @param charset         字符集，如UTF-8, GBK, GB2312
     * @param isEncrypt       是否加密，true-加密  false-不加密
     * @param isSign          是否签名，true-签名  false-不签名
     * @param signType        指定采用的签名方式，RSA、RSA2、SM2
     * @return 加密、签名后xml内容字符串 <p> 返回示例： <KiplePay> <response>密文</response> <encryption_type>RSA</encryption_type> <sign>sign</sign>
     * <sign_type>RSA</sign_type> </KiplePay> </p>
     * @throws KiplePayApiException
     */
    public static String encryptAndSign(String bizContent, String KiplePayPublicKey,
                                        String cusPrivateKey, String charset, boolean isEncrypt,
                                        boolean isSign, String signType) throws KiplePayApiException {
        ...
        if (isEncrypt) {// 加密
            sb.append("<KiplePay>");
            String encrypted = encrypt(bizContent, KiplePayPublicKey, charset, signType);
     ...
     * @throws KiplePayApiException
     */
    public static String checkSignAndDecrypt(Map<String, String> params, String KiplePayPublicKey,
                                             String cusPrivateKey, boolean isCheckSign,
                                             boolean isDecrypt, String signType) throws KiplePayApiException {
        String charset = params.get("charset");
        String bizContent = params.get("biz_content");
        if (isCheckSign) {
            if (!verifyV2(params, KiplePayPublicKey, charset, signType)) {
                throw new KiplePayApiException("rsaCheck failure:rsaParams=" + params);
            }
        }
    ...
    public static Map<String, String> getSortedMap(RequestParametersHolder requestHolder) {
        ...
        KiplePayHashMap appParams = requestHolder.getApplicationParams();
        if (appParams != null && appParams.size() > 0) {
            sortedParams.putAll(appParams);
        }
        KiplePayHashMap protocalMustParams = requestHolder.getProtocalMustParams();
        if (protocalMustParams != null && protocalMustParams.size() > 0) {
            sortedParams.putAll(protocalMustParams);
        }
        KiplePayHashMap protocalOptParams = requestHolder.getProtocalOptParams();
    ...
    public static boolean rsaCertCheckV1(Map<String, String> params, String KiplePayPublicCertPath,
                                         String charset) throws KiplePayApiException {
        String publicKey = getKiplePayPublicKey(KiplePayPublicCertPath);
    ...
    public static boolean rsaCertCheckV2(Map<String, String> params, String KiplePayPublicCertPath,
                                         String charset) throws KiplePayApiException {
        String publicKey = getKiplePayPublicKey(KiplePayPublicCertPath);
    ...
    public static boolean rsaCertCheck(String content, String sign, String KiplePayPublicCertPath, String charset,
                                       String signType) throws KiplePayApiException {
        String publicKey = getKiplePayPublicKey(KiplePayPublicCertPath);
        return rsaCheck(content, sign, publicKey, charset, signType);
    }
    ...
    public static String checkSignAndDecrypt(Map<String, String> params, String KiplePayPublicKey,
                                             String cusPrivateKey, boolean isCheckSign,
                                             boolean isDecrypt) throws KiplePayApiException {
        String charset = params.get("charset");
        String bizContent = params.get("biz_content");
        if (isCheckSign) {
            if (!rsaCheckV2(params, KiplePayPublicKey, charset)) {
                throw new KiplePayApiException("rsaCheck failure:rsaParams=" + params);
            }
        }
    ...
    public static String encryptAndSign(String bizContent, String KiplePayPublicKey,
                                        String cusPrivateKey, String charset, boolean isEncrypt,
                                        boolean isSign) throws KiplePayApiException {
        StringBuilder sb = new StringBuilder();
        if (StringUtils.isEmpty(charset)) {
            charset = KiplePayConstants.CHARSET_UTF8;
        }
        ...
        if (isEncrypt) {// 加密
            sb.append("<KiplePay>");
            String encrypted = rsaEncrypt(bizContent, KiplePayPublicKey, charset);
            sb.append("<response>" + encrypted + "</response>");
            sb.append("<encryption_type>RSA</encryption_type>");
            if (isSign) {
     ...
     * @throws KiplePayApiException
     */
    public static String getCertSN(String certPath) throws KiplePayApiException {
        return KiplePayCertificationUtil.getCertSN(KiplePayCertificationUtil.getCertFromPath(certPath));
    }

    /**
     * 从公钥证书中提取公钥
     *
     * @param KiplePayPublicCertPath 公钥证书存放路径，例如:/home/admin/cert.crt
     * @return 公钥
     * @throws KiplePayApiException
     */
    public static String getKiplePayPublicKey(String KiplePayPublicCertPath) throws KiplePayApiException {
        return KiplePayCertificationUtil.getKiplePayPublicKey(KiplePayPublicCertPath);
    }
}
