...
import com.kiple.api.domain.request.KiplePayFormRequest;
...
public class PaymentTest implements Runnable{
    ...
    public static class SandBox {
        ...
        //47.254.245.6647.254.241.181
        //192.168.1.240 request_body is =============>http://47.254.245.66
        public static final String GATEWAY = "http://47.254.245.66/payment/api/v1.0/topup/card";
    }
    /**
     * test request and wrapper the parameters
     */
    public void main(String[] agrs) throws KiplePayApiException {
        String url = SandBox.GATEWAY;
        String token = "3161e6fe-6126-4c6f-9f83-073f07c229bb";
        String content = "{" +
                "\"amount\":\"10\"," +
                "\"card_bin\":\"541060XXXXXX3563\"," +
                "\"merchant_order_sn\":\""+(System.currentTimeMillis()+"xx"+(new Random().nextInt(90000)+10000))+"\"," +
                "\"redirect_url\":\"https://www.baidu.com\"" +
                "}";
        String requestType = "post";
        if(agrs != null && agrs.length >3){
            url = agrs[0];
            token = agrs[1];
            content = agrs[2];
            requestType = agrs[3];
       ...
       // System.out.println(key);
        KiplePayClient kiplePayClient = new DefaultKiplePayClient(url,
...
//        request.addHeader("token","1893bec1-a245-472a-a779-2656fd5dd6e3");
        final KiplePayResponse<JSONObject> kiplePayResponse = kiplePayClient.execute(request,requestType);
        System.out.println("request:"+content+",result:"+JSONObject.toJSONString(kiplePayResponse));
    }
}
